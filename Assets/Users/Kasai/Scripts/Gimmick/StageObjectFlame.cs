...
public class StageObjectFlame : BaseStageObject
{
    ...
    {
        _flamePos = new Vector3(this.transform.position.x,0, this.transform.position.z);
    ...
    {
        _flamePos = this.transform.position;
        _destinationPos = _flamePos;
        _flamePos.y += 1f;
    ...
    private void FireMove(Vector2 direction)//炎の移動先の検索
    {
            ...
            stageManager.GetStageObjectType((int)_destinationPos.x, (int)_destinationPos.z) == null)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                if (!AreaEvent(stageManager.GetStageObjectType((int)_destinationPos.x, (int)_destinationPos.z)))//マウスと座標がかぶったときの処理も追加予定
                {
                    //巻き戻し
                ...
                else if (AreaEvent(stageManager.GetStageObjectType((int)_destinationPos.x, (int)_destinationPos.z)) || _destinationPos == mousePos)//マウスの座標とかぶったときとAreaEventがtrueのとき
                {
                    break;
                }
                RouteSearch(direction);
